...
- (void)relayoutToolsButton {
    ...
    _guestImage =[[UIImageView alloc] initWithFrame:CGRectMake(size.width/2-75, 100+addY, 150, 150)];
    //_gusetImage = [self.param.guestImg];
    NSURL *url = [NSURL URLWithString: self.guestimg];
    UIImage *image = [UIImage imageWithData: [NSData dataWithContentsOfURL:url]]; 
    _guestImage.image=image;

    _nickname = [[UILabel alloc]initWithFrame:CGRectMake(0, 260+addY, size.width, 20) ];
    _nickname.font = [UIFont systemFontOfSize:16];
    _nickname.numberOfLines = 1;
    _nickname.textColor = UIColor.whiteColor;
    _nickname.textAlignment = NSTextAlignmentCenter;
    _nickname.text = self.guestname;
    [self.view addSubview:_guestImage];
    ...
    _btnAudioMute.center = CGPointMake(startSpace + ICON_SIZE / 2, iconY);
    ...
    _btnHandfree.bounds = CGRectMake(0, 0, ICON_SIZE+10, ICON_SIZE+10);
...
- (void)enterRoom {
    ...
    // 进房
    [_trtc enterRoom:self.param appScene:TRTCAppSceneAudioCall];
