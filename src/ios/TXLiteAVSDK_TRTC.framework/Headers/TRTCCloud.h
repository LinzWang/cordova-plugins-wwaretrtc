...
/**
 * 9.15 将指定窗口加入屏幕分享的排除列表中，加入排除列表中的窗口不会被分享出去，仅适用 Mac 平台
 *
...
/**
 * 9.16 将指定窗口从屏幕分享的排除列表中移除，仅适用 Mac 平台
 *
 * @param windowID 不希望分享出去的窗口ID
 */
...
/**
 * 9.17 将所有窗口从屏幕分享的排除列表中移除，仅适用 Mac 平台
 */
...
/**
 * 10.8 设置音视频编码之后的数据自定义处理回调
 * @param listener 编码后的数据回调，listener为 C++ 回调对象的地址，listener = 0 则停止回调
 * @note: 详细使用方法请参见 TXLiteAVEncodedDataProcessingListener.h
 */
- (void)setEncodedDataProcessingListener:(void*)listener;
/// @}
